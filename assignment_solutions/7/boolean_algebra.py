...
def boolean(x: bool or int, y: bool or int, operation: str):
    operations = {
        # "conjunction"
        # > denoted x ∧ y,
        # > satisfies x ∧ y = 1 if x = y = 1 and x ∧ y = 0 otherwise.
        # "conjunction": lambda _x, _y: _x == 1 and _y == 1,  # pythonic
        "conjunction": lambda _x, _y: _x == 1 == _y,  # mathematical

        # "disjunction"
        # > denoted x ∨ y,
        # > satisfies x ∨ y = 0 if x = y = 0 and x ∨ y = 1 otherwise.
        # "disjunction": lambda _x, _y: _x == 1 or _y == 1,  # pythonic
        "disjunction": lambda _x, _y: not _x == 0 == _y,  # mathematical
        ...
        #   only two choices, so the return value is the one that entails
        #   less, namely true.
        "implication": lambda _x, _y: _y if _x else 1,  # pythonic
        ...
        "exclusive": lambda _x, _y: (_x + _y) % 2,  # mathematical

        # "equivalence"
        # > denoted x ≡ y and can be described as ¬ (x ⊕ y).
        # > It's true just when x and y have the same value.
        "equivalence": lambda _x, _y: _x == _y,  # mathematical
    }
    operation = operations[operation]
    return int(operation(x, y))


if __name__ == '__main__':
    # These "asserts" using only for self-checking and not necessary for auto-testing
    print(boolean(1, 0, "conjunction"), "and")
    assert boolean(1, 0, "conjunction") == 0, "and"

    print(boolean(1, 0, "disjunction"), "or")
    assert boolean(1, 0, "disjunction") == 1, "or"

    print(boolean(1, 1, "implication"), "material")
    assert boolean(1, 1, "implication") == 1, "material"

    print(boolean(0, 1, "exclusive"), "xor")
    assert boolean(0, 1, "exclusive") == 1, "xor"

    print(boolean(0, 1, "equivalence"), "same?")
